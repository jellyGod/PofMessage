...
public class OFFlowMod extends OFMessage implements OFInstructionFactoryAware, Cloneable {
    ...
    public void readFrom(ByteBuffer data) {
        ...
        this.matchFieldNum = data.get();
        this.instructionNum = data.get();
        data.get();
        ...
        this.tableType = OFTableType.values()[ data.get() ];
    ...
    public void writeTo(ByteBuffer data) {
        ...
        data.put(this.command);
        data.put(this.matchFieldNum);
        data.put(this.instructionNum);
        data.put((byte)0);
        data.putInt(this.counterId);
        
        data.putLong(this.cookie);
        data.putLong(this.cookieMask);
        
        data.put(this.tableId);
        data.put(this.tableType.getValue());
        ...
        if (this.matchList == null){
            data.put(new byte[OFGlobal.OFP_MAX_MATCH_FIELD_NUM * OFMatchX.MINIMUM_LENGTH]);
        }else{
            OFMatchX matchX;
            ...
            for(; i < matchFieldNum && i < OFGlobal.OFP_MAX_MATCH_FIELD_NUM; i++){
                ...
                if(matchX == null){
                    data.put(new byte[OFMatchX.MINIMUM_LENGTH]);
                }else{
                    matchX.writeTo(data);
                }
            }
            if(i < OFGlobal.OFP_MAX_MATCH_FIELD_NUM){
                data.put( new byte[(OFGlobal.OFP_MAX_MATCH_FIELD_NUM - i) * OFMatchX.MINIMUM_LENGTH]);
            }
        }        
        
        if (this.instructionList == null){
            data.put(new byte[OFGlobal.OFP_MAX_INSTRUCTION_NUM * OFInstruction.MAXIMAL_LENGTH]);
        }else{
            ...
            for(; (i < instructionNum) && (i < OFGlobal.OFP_MAX_INSTRUCTION_NUM); i++){
                ...
                if(instruction == null){
                    data.put(new byte[OFInstruction.MAXIMAL_LENGTH]);
                }else{
                    instruction.writeTo(data);
                    if(instruction.getLength() < OFInstruction.MAXIMAL_LENGTH){
                        data.put(new byte[OFInstruction.MAXIMAL_LENGTH - instruction.getLength()]);
                    }
                }
            }
            if(i < OFGlobal.OFP_MAX_INSTRUCTION_NUM){
                data.put( new byte[(OFGlobal.OFP_MAX_INSTRUCTION_NUM - i) * OFInstruction.MAXIMAL_LENGTH]);
            }
        }
    }
